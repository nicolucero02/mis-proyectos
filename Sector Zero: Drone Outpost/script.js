...
const ZOMBIE_TYPES = {
  NORMAL: {
    ...
    speed: 1.0,
    scale: 1.0,
    color: 0x2a4a5c, // Steel blue
    headColor: 0x4a5a6a, // Grey metal
    eyeColor: 0x00ddff, // Cyan visor
    detectRange: 60,
    attackRange: 3.5,
    chaseRange: 8,
    attackDamage: 10,
  ...
  FAST: {
    ...
    speed: 1.8,
    scale: 0.9,
    color: 0xffaa00, // Energy yellow
    headColor: 0x666666, // Grey
    eyeColor: 0xffdd00, // Bright yellow
    detectRange: 70,
    attackRange: 4.0,
    chaseRange: 10,
    attackDamage: 8,
  ...
  TANK: {
    ...
    speed: 0.6,
    scale: 1.4,
    color: 0x1a1a2a, // Dark gunmetal
    headColor: 0x333344, // Dark grey
    eyeColor: 0x00aaff, // Electric blue
    detectRange: 50,
    attackRange: 4.5,
    chaseRange: 8,
    attackDamage: 20,
  ...
  BOSS: {
    ...
    headColor: 0x1a1a2a, // Dark gunmetal
    eyeColor: 0xff00ff, // Magenta
    detectRange: 80,
    attackRange: 6.0,
    chaseRange: 12,
    attackDamage: 30,
...
// Event handler references for clean session destruction
let clickHandler, lockHandler, unlockHandler;
...
// Play electric buzz / servo sound for drone detection
function playZombieGrowlSound() {
  ...
  // Electric buzz: low sawtooth with fast LFO modulation
  ...
  gain.gain.linearRampToValueAtTime(effectsVolume * 0.25, now + 0.05);
...
// Play robot glitch / spark sound for drone attack
function playZombieAttackSound() {
  ...
  // Glitch spark: white noise burst + square beep
  ...
  noiseGain.gain.setValueAtTime(effectsVolume * 0.4, now);
  noiseGain.gain.exponentialRampToValueAtTime(0.001, now + 0.08);

  const beep = ctx.createOscillator();
  beep.type = "square";
  beep.frequency.setValueAtTime(1200, now);
  beep.frequency.exponentialRampToValueAtTime(200, now + 0.06);

  const beepGain = ctx.createGain();
  beepGain.gain.setValueAtTime(effectsVolume * 0.2, now);
...
function createBurnedBuildings() {
  for (let i = 0; i < 2; i++) {
    ...
    // Set dark background with thin cyan border
    ...
    // Add sci-fi warning text for each building
    if (i === 0) {
      // First building: Warning signs
      ctx.font = "bold 80px Arial";
      ctx.fillText("SECTOR ZERO", canvas.width / 2, 140);

      ctx.font = "bold 48px Arial";
      ctx.fillText("DEFENSE OUTPOST", canvas.width / 2, 220);

      ctx.font = "bold 36px Arial";
      ctx.fillText("THREAT LEVEL: ELEVATED", canvas.width / 2, 290);
      ctx.fillText("AUTHORIZED ACCESS ONLY", canvas.width / 2, 340);
    } else {
      // Second building: Patrol warnings
      ctx.font = "bold 80px Arial";
      ctx.fillText("RESTRICTED AREA", canvas.width / 2, 160);

      ctx.font = "bold 48px Arial";
      ctx.fillText("PATROL ACTIVE", canvas.width / 2, 240);

      ctx.font = "bold 36px Arial";
      ctx.fillText("HOSTILE UNITS DETECTED", canvas.width / 2, 310);
    }
    ...
    // Position the sign clearly on the front wall, above windows
    sign.position.set(0, 2.5, buildingGeometry.parameters.depth / 2 + 0.15);
...
// Create a drone/mech model based on type
function createZombieModel(zombieType) {
  const typeData = ZOMBIE_TYPES[zombieType];
  const zombieGroup = new THREE.Group();

  // Main chassis (wide, low-profile security unit body)
  ...
  // Top sensor dome (flattened optical sensor)
  ...
  // Wide panoramic visor (security unit eye band)
  const visorGeometry = new THREE.BoxGeometry(0.4, 0.05, 0.14);
  ...
  zombieGroup.add(visor);

  // Front-facing LED status lights
  ...
  leftLed.position.set(-0.25, 0.5, 0.32);
  zombieGroup.add(leftLed);
  const rightLed = new THREE.Mesh(ledGeometry, ledMaterial);
  rightLed.position.set(0.25, 0.5, 0.32);
  zombieGroup.add(rightLed);

  // Side-mounted weapon pods (instead of human arms)
  const podGeometry = new THREE.BoxGeometry(0.25, 0.35, 0.25);
  ...
  zombieGroup.add(leftPod);

  const rightPod = new THREE.Mesh(podGeometry, podMaterial);
  rightPod.position.set(0.5, 0.5, 0);
  zombieGroup.add(rightPod);

  // Weapon barrels on pods
  const barrelGeometry = new THREE.CylinderGeometry(0.04, 0.04, 0.3, 6);
  ...
  leftBarrel.position.set(-0.5, 0.5, 0.2);
  zombieGroup.add(leftBarrel);

  const rightBarrel = new THREE.Mesh(barrelGeometry, barrelMaterial);
  rightBarrel.rotation.x = Math.PI / 2;
  rightBarrel.position.set(0.5, 0.5, 0.2);
  zombieGroup.add(rightBarrel);

  // Three thick mechanical legs (heavy-duty security unit base)
  const legGeometry = new THREE.CylinderGeometry(0.09, 0.07, 0.55, 6);
  ...
  // Special features based on drone type
  if (zombieType === "TANK" || zombieType === "BOSS") {
    // Heavy armor plating
    const armorGeometry = new THREE.BoxGeometry(0.9, 0.2, 0.6);
    ...
    // Shoulder plates
    const plateGeometry = new THREE.BoxGeometry(0.15, 0.25, 0.35);
    const plateMaterial = new THREE.MeshLambertMaterial({ color: 0x1a1a2a });

    const leftPlate = new THREE.Mesh(plateGeometry, plateMaterial);
    leftPlate.position.set(-0.55, 0.65, 0);
    zombieGroup.add(leftPlate);

    const rightPlate = new THREE.Mesh(plateGeometry, plateMaterial);
    rightPlate.position.set(0.55, 0.65, 0);
    zombieGroup.add(rightPlate);
  }

  if (zombieType === "FAST") {
    // Sensor radome on top
    const radomeGeometry = new THREE.SphereGeometry(0.15, 8, 8);
    const radomeMaterial = new THREE.MeshLambertMaterial({ color: 0x444444 });
    const radome = new THREE.Mesh(radomeGeometry, radomeMaterial);
    radome.position.y = 1.15;
    zombieGroup.add(radome);

    // Thin aerodynamic fins
    const finGeometry = new THREE.BoxGeometry(0.05, 0.3, 0.2);
    const finMaterial = new THREE.MeshLambertMaterial({ color: typeData.color });

    const leftFin = new THREE.Mesh(finGeometry, finMaterial);
    leftFin.position.set(-0.4, 0.5, -0.15);
    zombieGroup.add(leftFin);

    const rightFin = new THREE.Mesh(finGeometry, finMaterial);
    rightFin.position.set(0.4, 0.5, -0.15);
    zombieGroup.add(rightFin);
  }

  if (zombieType === "BOSS") {
    // Command antenna/turret on top
    const turretGeometry = new THREE.CylinderGeometry(0.1, 0.15, 0.3, 8);
    const turretMaterial = new THREE.MeshLambertMaterial({ color: 0x1a1a2a });
    const turret = new THREE.Mesh(turretGeometry, turretMaterial);
    turret.position.y = 1.25;
    zombieGroup.add(turret);

    // Glowing command light
    ...
    const commandLight = new THREE.Mesh(lightGeometry, lightMaterial);
    commandLight.position.y = 1.45;
    zombieGroup.add(commandLight);

    // Extra armor skirt
    const skirtGeometry = new THREE.BoxGeometry(1.0, 0.15, 0.7);
    const skirtMaterial = new THREE.MeshLambertMaterial({ color: 0x111111 });
    const skirt = new THREE.Mesh(skirtGeometry, skirtMaterial);
    skirt.position.y = 0.25;
    zombieGroup.add(skirt);
  }
...
function prepareZombieAttack(zombie, userData, pathToPlayer) {
  ...
  // Robotic units maintain stable height while targeting
  zombie.position.y = 0;
}
...
function executeZombieChase(zombie, userData, pathToPlayer) {
  ...
  // Mechanical vibration at high RPM during pursuit
  const time = Date.now() * 0.02;
  zombie.position.y = Math.abs(Math.sin(time)) * 0.03;
}
...
function executeZombieWander(zombie, userData) {
  ...
  // Security androids maintain stable posture while patrolling
  zombie.position.y = 0;
}
...
function drawZombiesOnRadar(playerPos, centerX, centerY) {
  zombies.forEach((zombie) => {
    ...
    let zombieColor = "rgba(0, 200, 255, 0.8)"; // Default cyan
    let zombieSize = 2;

    if (zombie.userData.zombieType) {
      switch (zombie.userData.zombieType) {
        case "FAST":
          zombieColor = "rgba(255, 165, 0, 0.8)"; // Orange for fast drones
          zombieSize = 1.5;
          break;
        case "TANK":
          zombieColor = "rgba(0, 100, 255, 0.8)"; // Blue for heavy mechs
          zombieSize = 3;
          break;
        case "BOSS":
          zombieColor = "rgba(255, 255, 255, 0.8)"; // White for command units
          zombieSize = 4;
          break;
        default:
          zombieColor = "rgba(0, 200, 255, 0.8)"; // Cyan for scout drones
          zombieSize = 2;
      }
    }
...
function initControls() {
  ...
  unlockHandler = function () {
    if (gameState === "playing") {
      // ESC released pointer lock — show resume prompt, don't pause game
      blocker.style.display = "block";
      instructions.style.display = "";
      const h1 = instructions.querySelector("h1");
      if (h1) h1.textContent = "SYSTEM STANDBY";
      const clickText = instructions.querySelector("strong");
      if (clickText) clickText.textContent = "Click to resume";
    }
  };
...
function updateKillCount() {
  ...
  killCountDiv.textContent = `Units Destroyed: ${zombieKills}`;
}
...
function updateHealthPackCount() {
  ...
  healthPackCountDiv.textContent = `Repair Kits: ${healthPacks.length}/${MAX_HEALTH_PACKS} | Collected: ${healthPacksCollected}`;
}
...
function updateWeaponDisplay() {
  ...
  weaponDiv.textContent = `${weapon.name} | ${fireRateText}${ammoText}`;
...
function updateHealthBar() {
  ...
  if (healthFill && healthText) {
    ...
    healthText.textContent = `Shield Integrity: ${playerHealth}/${maxHealth}`;
...
function createDamageEffect() {
  ...
  flashDiv.style.background = "rgba(0, 150, 255, 0.3)";
...
function updateWaveDisplay() {
  ...
  if (waveInProgress) {
    ...
    let status = `Threat Level ${currentWave} | Hostile Units: ${livingZombies}/${zombiesThisWave}`;
    ...
    Object.keys(typeBreakdown).forEach((type) => {
      if (typeBreakdown[type] > 0) {
        ...
        typeInfo.push(`${typeData.name.split(' ')[0]}:${typeBreakdown[type]}`);
      }
    });
    ...
    if (huntingZombies > 0) {
      status += ` | Scanning: ${huntingZombies}`;
    }
    if (chasingZombies > 0) {
      status += ` | Pursuing: ${chasingZombies}`;
    }
    if (attackingZombies > 0) {
      status += ` | Engaging: ${attackingZombies}`;
    }
    ...
    waveDiv.textContent = `Threat Level ${currentWave} initializing...`;
  }
}
...
function animate() {
  ...
  if (gameState === "playing" && !isGameOver) {
    ...
    // Animate environment objects (only when playing to avoid pause lag/flicker)
...
// Track where instructions were opened from
let instructionsReturnTarget = "mainMenu";
...
function showInstructions() {
  ...
  // Update back button based on where we came from
  const backBtn = document.getElementById("instructionsBackBtn");
  if (backBtn) {
    if (instructionsReturnTarget === "pause") {
      backBtn.textContent = "BACK TO PAUSE";
    } else {
...
function startGame() {
  ...
  // Initialize game — after destroyGameSession() this is always a fresh init
  if (!gameInitialized) {
    init();
    gameInitialized = true;
  } else {
    // Fallback: should not happen now that quit destroys the session
    resetGameState();
    startWaveSystem();
  }
...
function resumeGame() {
  ...
  // Restore pointer lock automatically so player can resume immediately
...
function destroyGameSession() {
  console.log("Destroying game session...");
  ...
  // 4. Remove all document/window event listeners
  if (clickHandler) {
    const instructions = document.getElementById("instructions");
    if (instructions) instructions.removeEventListener("click", clickHandler);
  }
  if (controls) {
    if (lockHandler) controls.removeEventListener("lock", lockHandler);
    if (unlockHandler) controls.removeEventListener("unlock", unlockHandler);
  }
  ...
  // 5. Tear down Three.js renderer and scene
  if (renderer) {
    renderer.dispose();
  ...
  // 6. Null out all session objects so init() rebuilds them fresh
  ...
  zombieTypeCounts = { NORMAL: 0, FAST: 0, TANK: 0, BOSS: 0 };
...
function quitToMainMenu() {
  // Fully destroy the current session so the next Start Mission is a fresh load
  destroyGameSession();
...
function resetGameState() {
  ...
  // Clear any lingering spawn timers so they don't duplicate on restart
  if (zombieSpawnTimer) {
    clearTimeout(zombieSpawnTimer);
    zombieSpawnTimer = null;
  }
...
window.addEventListener("load", () => {
  ...
  // Don't start animation loop here — init() will start it when the user clicks Start Mission
});
...
function toggleMute() {
  ...
  const pauseSlider = document.getElementById("pauseVolume");

  if (isMuted) {
    previousVolume = masterSlider ? masterSlider.value : 100;
    ...
    updateMasterVolume(previousVolume);
    if (masterSlider) masterSlider.value = previousVolume;
    if (pauseSlider) pauseSlider.value = previousVolume;
...
window.returnFromInstructions = returnFromInstructions;
